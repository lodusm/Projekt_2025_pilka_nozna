...
def get_player_info(player_id):
    ...
    if player_row.empty:
        return None, None
    return player_row.iloc[0].to_dict(), lineups[lineups['player_id'] == player_id]
...
def get_related_data(player_id, team):
    ...
    starting_events = pd.DataFrame(list(db.events.find({'type': 'Starting XI', 'team': team})))
    return events, matches, starting_events

def extract_player_events(events, player_id):
    player_events = events[(events['player_id'] == player_id) & events['location'].notna()].copy()
...
def generate_stats_table(player_events, events, minutes_played, player_id):
    ...
    def safe_per_90(value):
        return round(value * 90 / minutes_played, 2) if minutes_played > 0 else 0.0

    total_passes = count_event(player_events, 'Pass')
    completed_passes = count_event(player_events, 'Pass', lambda df: df['pass_outcome'].isna())
    dribbles = count_event(player_events, 'Dribble')
    successful_dribbles = count_event(player_events, 'Dribble', lambda df: df['dribble_outcome'] == 'Complete')
    total_shots = count_event(player_events, 'Shot')
    goals = count_event(player_events, 'Shot', lambda df: df['shot_outcome'] == 'Goal')
    ...
    if 'type' in events.columns and 'player_id' in events.columns:
        touches = events[
            ...
            (events['type'].isin(['Pass', 'Carry', 'Dribble', 'Shot', 'Interception', 'Clearance', 'Block', 'Foul Won']))
        ].shape[0]

    df_stats = pd.DataFrame({
        'Stat': ['Passes', 'Pass accuracy', 'Dribbles', 'Shots', 'Goals', 'Fouls committed', 'Fouls received', 'Ball touches', 'Carries'],
        'Total': [
            total_passes,
            f"{round(100 * completed_passes / total_passes, 2)}%" if total_passes > 0 else "0%",
            f"{successful_dribbles}/{dribbles}",
            total_shots,
            goals,
            fouls_committed,
            fouls_won,
...
def calculate_minutes(events_df, starting_df, player_id):
    ...
    for match_id in events_df['match_id'].unique():
        ...
        sub_in = match_events[
            (match_events['type'] == 'Substitution') &
            (match_events.get('substitution_replacement_id') == player_id if 'substitution_replacement_id' in match_events.columns else False)
...
def draw_map(player_events, shots, position_counts):
    ...
    fig = add_heatmap(fig, smoothed, colorscale='YlOrRd', opacity=0.9, showscale=False, showlegend=True, name='Activity', hoverinfo="skip")
    ...
    if position_counts:
        ...
        fig.add_trace(go.Scatter(
            ...
            text=[f"{pos}<br>{count}" for pos, count in zip(df["Position"], df["Count"])],
...
def layout(player_id=None):
    ...
    starting_appearances, sub_appearances = calculate_appearances(events, starting_events, player_id, position_counts)
    ...
    return html.Div([
    html.H1(f"{nickname}", style={"textAlign": "center", "marginBottom": "1rem"}),
    html.P(f"  👕Team: {team}       📍Nationality: {country}       #️Jersey: {jersey}", style={"textAlign": "center", "fontSize": "20px"}),

    html.Div([
        #występy
        dash_table.DataTable(
        columns=[{"name": col, "id": col} for col in appearance_data.columns],
...
html.Div([
    #tabelka
    html.Div([
        html.H4("📊 Season Stats", style={"textAlign": "center", "marginBottom": "40px"}),
        dash_table.DataTable(
