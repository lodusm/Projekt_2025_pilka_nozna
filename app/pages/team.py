...
def get_match_result_stats(events, matches_df, team_name):
    team_matches = matches_df[
        (matches_df['home_team'] == team_name) | (matches_df['away_team'] == team_name)
    ].copy()

    played = len(team_matches)

    def is_win(row):
        return row['home_score'] > row['away_score'] if row['home_team'] == team_name else row['away_score'] > row['home_score']
...
def get_scoring_offensive_stats(events, matches_df, team_name):
    ...
    clean_sheets = sum(
        (row['away_score'] == 0 if row['home_team'] == team_name else row['home_score'] == 0)
        for _, row in team_matches.iterrows()
    )
    failed_to_score = sum(
        (row['home_score'] == 0 if row['home_team'] == team_name else row['away_score'] == 0)
        for _, row in team_matches.iterrows()
    )
    ...
    accuracy = f"{(len(on_target) / len(shots)) * 100:.1f}%" if len(shots) > 0 else "0.0%"
    ...
    possession_acts = events[events['type'].isin(['Pass', 'Carry', 'Ball Receipt'])]
    ...
    return pd.DataFrame({
        "Stat": [
            "Goals Scored", "Goals Conceded", "Goal Difference", "Clean Sheets", "Failed to Score",
            "Total Shots", "Shots on Target", "Shot Accuracy", "Expected Goals (xG)",
            "Passes Attempted", "Passes Completed", "Pass Accuracy", "Average Possession"
        ],
        "Value": [
            goals_scored, goals_conceded, goal_diff, clean_sheets, failed_to_score,
            len(shots), len(on_target), accuracy, round(xg, 2), total_passes, len(completed), pass_acc, avg_pos
        ]
    })
...
def get_passing_possession_stats(events, team_name):
    ...
    return pd.DataFrame({
        "Stat": ["Passes Attempted", "Passes Completed", "Pass Accuracy", "Average Possession"],
        "Value": [total_passes, len(completed), pass_acc, avg_pos]
    })
...
def get_team_players(team_name):
    players = (
        lineups[lineups["team"] == team_name][["jersey_number", "player_name", "player_id"]]
        .drop_duplicates("player_id")
        .sort_values("jersey_number")
        .rename(columns={"jersey_number": "Jersey", "player_name": "Name"})
    )
...
def get_team_matches(team_name):
    ...
    table_data = team_matches[["match_id", "match_week", "Result", "home_team", "Score", "away_team"]].rename(columns={
        "match_week": "Week",
        "home_team": "Home",
        "away_team": "Away"
    })
...
def get_top_scorers(events, team_name=None):
    ...
    df = (
        goals.groupby(['player']).size().reset_index(name='Goals')
        .merge(penalty_goals.groupby('player').size().reset_index(name='Penalties'), on='player', how='left')
        .merge(free_kick_goals.groupby('player').size().reset_index(name='Free Kicks'), on='player', how='left')
        .fillna(0)
    )
    ...
    df.index += 1
    return df.rename(columns={'player': 'Player'})[['Player', 'Goals', 'Penalties', 'Free Kicks', 'Open Play']]
...
def draw_goals_treemap(events, team_name=None):
    ...
    return html.Div([
        dcc.Graph(
            ...
            style={"height": "770px", "width": "770px", "margin": "0 auto"}
        )
    ], style={
        "maxWidth": "600px",
        "maxHeight": "600px"
...
def layout(team_id=None):
    ...
    events = pd.DataFrame(list(db.events.find({"match_id": {"$in": match_ids}})))
    ...
    scoring_offensive_stats = get_scoring_offensive_stats(events, matches, team_name)
    ...
    return html.Div([
        ...
        dash_table.DataTable(
            columns=[{"name": col, "id": col} for col in match_result_stats.columns],
            data=[match_result_stats.iloc[0].to_dict()],
            style_table={"margin": "1rem auto", "maxWidth": "1400px", "overflowX": "auto", "borderRadius": "8px", "boxShadow": "0 0 8px rgba(0,0,0,0.3)"},
            style_cell={**common_cell_style, 'minWidth': '100px', 'width': '100px', 'maxWidth': '100px'},
            ...
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    style_data_conditional=[
                        {'if': {'row_index': 'odd'}, 'backgroundColor': '#1e2a3e'},
                        {'if': {'state': 'active'}, 'backgroundColor': '#2a3b50'},
                        {'if': {'column_id': 'player_id'}, 'display': 'none'}
                    ],
                    style_header_conditional=[
                        {'if': {'column_id': 'player_id'}, 'display': 'none'}
                    ],
        ...
        html.Div([
            html.Div([
                ...
                dash_table.DataTable(
                    columns=[{"name": col, "id": col} for col in scorers_df.columns],
                ...
                html.H4("Top Assistants", style={"textAlign": "center", "marginTop": "2rem"}),
                dash_table.DataTable(
                    columns=[{"name": col, "id": col} for col in assists_df.columns],
            ...
            html.Div([
                html.H4("Goals by player", style={"textAlign": "center", "marginTop": "2rem"}),
        ...
        html.Div([
            html.Div([
                ...
                dash_table.DataTable(
                    columns=[{"name": col, "id": col} for col in scoring_offensive_stats.columns],
            ...
            html.Div([
                html.H4("Shots Map", style={"textAlign": "center", "marginLeft": "-300px"}),
                dcc.Graph(
                    figure=draw_team_shot_map(events, team_name),
                    config={"displayModeBar": False},
...
@callback(
    Output('team-url', 'pathname', allow_duplicate=True),
