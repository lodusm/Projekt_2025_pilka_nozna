...
def generate_timeline(match_id):
    ...
    for _, row in goals.iterrows():
        ...
        if row['type'] == 'Shot':
            ...
            event_type = 'Goal (Pen)' if row.get('shot_type') == 'Penalty' else 'Goal'
        else:
            player_str = player
            event_type = 'Own Goal'
    ...
    for _, row in subs.iterrows():
        timeline.append({
            ...
            'minute': row['minute'],
            'team': row.get('team'),
            'player': f"⬇️ {row.get('player')} ⬆️ {row.get('substitution_replacement')}",
            'type': 'Substitution'
        })
...
def draw_lineup_plot(match_id):
    ...
    for _, row in starting_events.iterrows():
        ...
        for player in row['tactics']['lineup']:
            ...
            players.append({'x': x, 'y': 80 - y, 'team': team, 'name': name, 'number': number})
    ...
    for team, color in [(home_team, "blue"), (away_team, "red")]:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=40, color=color, line=dict(color="white", width=3)),
            textfont=dict(color="white", size=20),
            hovertext=team_df['name'],
            hoverinfo="text",
            showlegend=False
        ))
...
def generate_match_stats(events, lineups, match, home_team, away_team):
    ...
    def count_yellow_cards(team):
        return sum(1 for p in lineups_dict.get(team, []) for c in p.get("cards", []) if c.get("card_type") in ["Yellow Card", "Second Yellow"])

    def count_red_cards(team):
        return sum(1 for p in lineups_dict.get(team, []) for c in p.get("cards", []) if c.get("card_type") in ["Red Card", "Second Yellow"])
    ...
    stats = {
        ...
        'Shots': [count_shots(home_events), count_shots(away_events)],
        'Shots on Target': [count_shots_on_target(home_events), count_shots_on_target(away_events)],
        'xG': [round(sum_xg(home_events), 2), round(sum_xg(away_events), 2)],
        'Possession': [home_possession, away_possession],
        'Accurate Passes': [count_accurate_passes(home_events), count_accurate_passes(away_events)],
        'Pass Accuracy': [pass_accuracy_pct(home_events), pass_accuracy_pct(away_events)],
        'Corners': [count_corners(home_events), count_corners(away_events)],
        'Penalties': [count_penalties(home_events), count_penalties(away_events)],
        'Free Kicks': [count_free_kicks(home_events), count_free_kicks(away_events)],
        'Yellow Cards': [count_yellow_cards(home_team), count_yellow_cards(away_team)],
        'Red Cards': [count_red_cards(home_team), count_red_cards(away_team)],
    }

    df = pd.DataFrame.from_dict(stats, orient='index', columns=[home_team, away_team]).reset_index().rename(columns={'index': 'Statistic'})
    ...
    def parse_players(team, is_sub=False):
        ...
        for _, row in df.iterrows():
            players.append({
                ...
                'Name': row.get('player_name', row.get('player', {}).get('name')),
...
def draw_shot_map(events, home_team, away_team):
    ...
    shots = events[(events['type'] == 'Shot') & (events['location'].notnull())].copy()
...
def draw_pass_network(events, team_name):
    ...
    #pary zawodników
    ocmbinations = passes.groupby(['player', 'pass_recipient']).size().reset_index(name='count')
    ...
    for player, (x, y) in avg_pos.items():
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=14, color="blue", line=dict(color="white", width=2)),
...
def draw_xg_timeline(events, home_team, away_team):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers',
        marker=dict(symbol='star', size=12, color='blue', line=dict(color='white', width=1)),
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers',
        marker=dict(symbol='star', size=12, color='red', line=dict(color='white', width=1)),
...
def layout(match_id=None):
    ...
    home_df, away_df, subs_home, subs_away = get_lineup_tables(events, lineups, home_team, away_team)

    return html.Div([
        html.Div([
            html.Div(link_team(home_team), style={"flex": "1", "textAlign": "right", "paddingRight": "1rem", "fontSize": "4rem", "fontWeight": "bold"}),
            html.Div(f"{row['home_score']} - {row['away_score']}", style={"flex": "0", "textAlign": "center", "fontWeight": "bold", "fontSize": "4rem", "minWidth": "150px"}),
            html.Div(link_team(away_team), style={"flex": "1", "textAlign": "left", "paddingLeft": "1rem", "fontSize": "4rem", "fontWeight": "bold"})
                ], style={
                    "display": "flex",
        ...
        html.P(
            f"📅 {row.get('match_date', 'Unknown')} 🏟️  {row.get('stadium', 'Unknown')} ✏️ {row.get('referee', 'Unknown')}",
            ...
            html.Div([
                # LEFT - Home team
                html.Div([
                    html.H5("Starting XI", style={"textAlign": "center"}),
                    html.Div(
            dash_table.DataTable(
                columns=[{"name": col, "id": col} for col in home_df.columns],
            ...
            style={
                ...
                'overflowX': 'auto'
                    ...
                    html.H5("Bench", style={"marginTop": "1rem", "textAlign": "center"}),
                    html.Div(
            dash_table.DataTable(
                columns=[{"name": col, "id": col} for col in subs_home.columns],
            ...
            style={
                ...
                html.Div([
                    ...
                    html.Div(
            dash_table.DataTable(
                columns=[{"name": col, "id": col} for col in away_df.columns],
            ...
            style={
                ...
                'overflowX': 'auto'
                    ...
                    html.Div(
            dash_table.DataTable(
                columns=[{"name": col, "id": col} for col in subs_away.columns],
            ...
            "boxSizing": "border-box"
                ...
                # Row 2: Timeline + Shot map
                    html.Div([
                        # Timeline Block
                        html.Div([
                            html.H4("Timeline", style={"textAlign": "center"}),
                            dash_table.DataTable(
                                columns=[{"name": col.capitalize(), "id": col} for col in timeline_df.columns],
            ...
            html.Div([
                ...
                dcc.Graph(
                    figure=draw_shot_map(events, home_team, away_team),
                    config={"displayModeBar": False},
            ...
            html.Div([
                dash_table.DataTable(
                    ...
                    style_cell_conditional=[
                        {'if': {'column_id': stats_df.columns[0]}, 'width': '33.33%'},
                        {'if': {'column_id': stats_df.columns[1]}, 'width': '33.33%'},
                        {'if': {'column_id': stats_df.columns[2]}, 'width': '33.33%'},
                    ],
        ...
        html.Div([
            ...
            html.Div(draw_pass_network(events, home_team), style={
                "flex": "0 1 45%",
                "textAlign": "center",
                "display": "flex",
                "justifyContent": "center"
            }),
            html.Div(draw_pass_network(events, away_team), style={
                "flex": "0 1 45%",
                "textAlign": "center",
                "display": "flex",
                "justifyContent": "center"
        ...
        html.Div([
            html.H3("Match momentum", style={"textAlign": "center", "marginTop": "2rem"}),
            dcc.Graph(
                figure=draw_xg_timeline(events, home_team, away_team),
                config={"displayModeBar": False},
