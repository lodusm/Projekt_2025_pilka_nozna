...
navbar = dbc.Navbar(
    dbc.Container([
        dbc.Row([
            dbc.Col(dbc.NavbarBrand("⚽ LaLiga 2015/16", href="/", className="brand-title"), width="auto"),

            dbc.Col(dbc.Nav([
                dbc.NavItem(dbc.NavLink("Matches", href="/matches", className="nav-link")),
...
app.layout = html.Div([
    navbar,
    dcc.Location(id='url-dropdown', refresh=True),
    dash.page_container
], className="app-body")
...
def update_player_dropdown(selected_team):
    if selected_team:
        filtered = lineups[lineups['team'] == selected_team]['player_name'].dropna().unique()
